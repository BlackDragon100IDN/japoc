...
const DEFAULT_ADDRESS = 'mumble.madirc.net';
const DEFAULT_PORT = '';
...
function ConnectDialog () {
  var self = this
  self.address = ko.observable(DEFAULT_ADDRESS)
  self.port = ko.observable(DEFAULT_PORT)
...
class GlobalBindings {
  constructor () {
    ...
    this.connect = (username, host, port, token, password) => {
      ...
      // STRML: Force push-to-talk
      log('--')
      log('--NOTE: Push-To-Talk is on. Hold ctrl-shift to talk.')
      log('--')
      log('')
    ...
    this.requestUnmute = user => {
      if (this.connected()) {
        if (user === this.thisUser) {
          if (!this._keyComboPressed) return; // STRML: forced push to talk
    ...
    // STRML: forced push to talk
    this._setKeybinds = () => {
      // Mute self
      this.requestMute(this.thisUser)

      if (this._keyPressListener) return;

      window.KeyboardJS = KeyboardJS;
      this._keyPressListener = () => {
        this._keyComboPressed = true;
        this.client.self.__ui.talking('on')
        this.requestUnmute(this.thisUser);
      };
      this._keyReleaseListener = () => {
        this._keyComboPressed = false;
        this.client.self.__ui.talking('off')
        this.requestMute(this.thisUser);
      }
      KeyboardJS.bind('ctrl + shift', this._keyPressListener, this._keyReleaseListener);
    };
